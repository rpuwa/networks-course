...
#### Демонстрация работы
Демонстрируется с помощью скрипта multirequest.py, делающий запросы через клиентов из задания В
...
#### Решение
Один пакет хост А создаёт $d_{\text{обр}} = \frac{56 * 8}{128 * 1000} = \frac{7}{2000}$ секунды, или $3.5$ мс. 
Такой пакет передаётся в хост Б $d_{\text{перед}} = \frac{56 * 8}{1 * 1000000} * 0.001 = \frac{56}{125} = 0.448$ мс. 
Учитывая задержку распространения в $d_{\text{распр}} = 5$ мс, получаем время от кодирования до декодирования $d = d_{\text{обр}} + d_{\text{перед}} + d_{\text{распр}} = 8.948$ мс.
...
$d = 10 \text{ (задержка ожидания)} + \frac{1000}{100} \text{ (задержка передачи, мс)} = 20$ мс.

$N = 10 \text{ (в среднем в буфере)} + 1 \text{ (в передаче)} = 11$ пакетов

$a = \frac{N}{d} = \frac{11}{20} = 0.55$ пакет/мс.
...
#### Решение
1. Первый пакет будет лететь $d_1 = \frac{L}{R_S} + d_{\text{распространения}} + d_{\text{обработки}} + \frac{L}{R_C} + d_{\text{распространения}}$ секунд. Второй же пакет прилетит спустя $d_2 = \frac{L}{R_S} \text{ (задержка ожидания на сервере)} + \frac{L}{R_S} + d_{\text{распространения}} + d_{\text{обработки}} + \frac{L}{R_C} + d_{\text{распространения}} = \frac{L}{R_S} + d_1$, откуда ответ $\frac{L}{R_S}$.
2. Может, поскольку он прилетит спустя $\frac{L}{R_S}$ секунд, и первый пакет ещё не успеет полностью отправиться: ему останется $\frac{L}{R_C} - \frac{L}{R_S} > 0$ секунд. Чтобы задержки не было, нужно, чтобы второй пакет отправился через $\frac{L}{R_C} - \frac{L}{R_S}$ секунд.
