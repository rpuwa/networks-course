...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - русский, английский
   - поддерживаемые кодировки, тип контента, юзер-агент
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   ...
   - вернул
   - видно во вкладке Line-based text data
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - вижу, там написана дата изменения файла на сервере, совпадает с полем Last-Modified из первого ответа сервера
...
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Content-length поле передаёт размер ответа, а так информация о том, что пакетов 4 не передана в HTTP ответе
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3
   - 128.119.245.12 (gaia.cs.umass.edu) и 178.79.137.164 (kurose.cslash.net)
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Параллельно. Последовательными были лишь запросы 32 (получить сам сайт) и 37 (получить картинку pearson.png), запрос за 8E_cover_small.jpg был отдельным, и не зависел от других запросов
